...
class LlavaMetaForCausalLM(ABC):
    ...
    def token_merging(self, image_features, errors_indices, scaling=1):
        """
        ...
        for b in range(B):
            merged_features.append(image_features[b][errors_indices[b]])
    ...
    def encode_videos(self, videos):  # [mini_b, c, t, h, w]
        ...
        video_cls_features = video_features[:, 0, :]  # (8, 4096)
        video_features = video_features[:, 1:, :]

        seed_features = video_features.gather(1, fps(video_features, 10).unsqueeze(-1).expand(-1, -1,
                                                                                                                 video_features.shape[
                                                                                                                     -1]))
        seed_features = seed_features.float()
        video_features = video_features.float()
        G = torch.matmul(seed_features, seed_features.transpose(-1, -2))  # [B, k, k]
        G.diagonal(dim1=-2, dim2=-1).add_(1e-5)
        B_rhs = torch.matmul(seed_features, video_features.transpose(-1, -2))  # [B, k, N]
        linear_group = torch.linalg.solve(G, B_rhs).transpose(-1, -2)  # [B, N, k]
        image_feature_hat = torch.matmul(linear_group, seed_features)  # [B, N, d]
        errors = torch.norm(video_features - image_feature_hat, dim=-1)  # [B, N]
        errors_indices = torch.topk(errors, k=22, dim=1)[1]  # [B, visual_token_num]
        video_features = video_features.half()
        seed_features = seed_features.half()

        video_features = self.token_merging(video_features, errors_indices) # (t, 22, 4096)
        video_cls_features = video_cls_features.unsqueeze(1)
        video_features = torch.cat([video_cls_features, video_features, seed_features], dim=1)
